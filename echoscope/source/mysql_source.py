...
class MysqlSource(source.Source):
  ...
  def exportModel(self, conf: config_model.DataSourceConfig) -> ds_model.DataSourceModel:
    ...
    ver = self.getMysqlVersion(mysqlUtil)
    if ver == '':
      logging.error(' mysql conn fail. ')
      return
    dsm = ds_model.DataSourceModel(
        name='%s:%d' % (conf.host, conf.port), dbType='mysql', version=ver)

    dsm.dbs = self.getExportDbs(mysqlUtil, conf.includes, conf.excludes)

    dsm = self.fillTableFields(mysqlUtil, dsm)

    return dsm

  def getMysqlVersion(self, conn: mysql_util.MysqlUtil) -> str:
    """获取mysql版本
  ...
  def getExportDbs(self, conn: mysql_util.MysqlUtil, includes: List[str] = [], excludes: List[str] = []) -> List[ds_model.DbModel]:
    """获取需要导出结构的数据库列表
  ...
  def fillTableFields(self, conn: mysql_util.MysqlUtil, dsModel: ds_model.DataSourceModel) -> ds_model.DataSourceModel:
    """获取数据库中的表信息
    ...
    for db in dsModel.dbs:
      ...
      for d in data:
        ...
        table.fields = self.getFields(conn, db.name, tableName)
        tables.append(table)
      db.tables = tables

    return dsModel

  def getFields(self, conn: mysql_util.MysqlUtil, dbName: str, tableName: str) -> List[ds_model.FieldModel]:
    """获取数据表中列信息
